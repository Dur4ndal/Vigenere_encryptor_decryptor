...
#Handler de erro de base da cifra
def verificador(texto, chave):
        if ((len(texto)) != (len(chave))):
                print ("O texto plano, texto cifrado e chave devem possuir o mesmo tamanho")
                exit()

#Cifrador
# O(n)
def cifrador(textoplano, chave):
        #Limpa espaços
        try:
                while True:
                        textoplano.remove(' ')
        ...
        verificador(textoplano, chave)
        Index_key = 0
        for C_plaintext in textoplano:
...
#Decifrador
# O(n)
def decifrador(textocifrado, chave):
        #Limpa espaços
        try:
                while True:
                        textocifrado.remove(' ')
                ...
                pass
        verificador(textocifrado, chave)
        Index_key = 0
        for C_ciphertext in textocifrado:
...
#Main
opt = int(input("Qual opcao:\n (1) - Cifrador\n (2) - Decifrador\n (3) - Brute-forcer\n"))

if ((opt == 1) or (opt == 2)):
        key = list(input('Digite a chave\n'))
        if (opt == 1):
                cifrador(list(input('Digite o texto plano\n')), key)

        elif (opt == 2):
                decifrador(list(input('Digite a cifra\n')), key)

elif (opt == 3):
        idioma = int(input('Escolha o idioma:\n (0) - Ingles\n (1) - Portugues\n'))
        if ((idioma == 1) or (idioma == 0)):
                ataque(input('Digite o texto a ser atacado\n'), idioma)
